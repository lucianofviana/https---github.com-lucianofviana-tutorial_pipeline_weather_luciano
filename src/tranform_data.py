...
import logging
logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(mensage)s')

path_name = Path(__file__).parent.parent /'data'/'weather_data_json'
...
def create_dataframe (path_name:str) -> pd.DataFrame:
	...
	if not path.exists():
		...
		raise FileNotFoundError(f"Arquivo não econtrado: {path}")
...
def normalize_weather_columns(df:pd.DataFrame)-> pd.DataFrame:
	...
	df_weather = pd.json_normalize(df['weather'].apply(lambda x:[0]))
